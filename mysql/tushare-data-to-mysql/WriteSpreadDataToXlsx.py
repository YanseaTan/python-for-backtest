...
def test_funds_multiyear(fut_code, index_name):
    # 以年为单位的数据组数
    cnt_of_year = 4
    ...
    for i in range(0, cnt_of_year):
        ...
        close_dict = {}
        for k in range(0, len(df)):
            if df.loc[k]['update_date'][2:4] > start_year[i]:
    ...
    for j in range(0, len(date_list)):
        ...
        close_list = [date_str] + [''] * cnt_of_year
        for k in range(0, cnt_of_year):
            if date in comb_dict[k]:
                close_list[k + 1] = comb_dict[k][date]
    ...
    if len(wb.sheets) > 1:
        wb.sheets['Sheet1'].delete()
    if not os.path.exists('output/{}/所有品种基本面数据季节性走势/'.format(todayStr)):
        os.makedirs('output/{}/所有品种基本面数据季节性走势/'.format(todayStr))
    wb.save('./output/{}/所有品种基本面数据季节性走势/{}-{} 品种基本面数据走势.xlsx'.format(todayStr, todayStr, fut_code))
    wb.close()
    app.quit()
    print('{} 品种基本面数据走势 Excel 数据导出完毕！'.format(fut_code))
