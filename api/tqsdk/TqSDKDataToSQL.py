...
def export_tqsdk_stock_daily_data_to_csv(api):
    ...
    for i in range(0, len(ts_code_df)):
        ...
        kd = DataDownloader(api, symbol_list=ts_code, dur_sec=86400,
                        start_dt=datetime(2018, 1, 1, 6, 0 ,0), end_dt=datetime(2024, 3, 18, 16, 0, 0), csv_file_name="./doc/daily-kline/stock/{}.daily.csv".format(ts_code))
        
        while not kd.is_finished():
            api.wait_update()
            print("{} progress: kline: %.2f%%".format(ts_code) % (kd.get_progress()))
        
        print("{} 日 k 线导出完毕，进度：{}%".format(ts_code, round((i + 1) / len(ts_code_df) * 100, 2)))
...
def export_tqsdk_index_daily_data_to_csv(api):
    code_list = ['SSE.000016', 'SSE.000300', 'SSE.000905', 'SSE.000852']
    for i in range(0, len(code_list)):
        code = code_list[i]
        kd = DataDownloader(api, symbol_list=code, dur_sec=86400,
                        start_dt=datetime(2018, 1, 1, 6, 0 ,0), end_dt=datetime(2024, 3, 18, 16, 0, 0), csv_file_name="./doc/daily-kline/index/{}.daily.csv".format(code))
        while not kd.is_finished():
            api.wait_update()
...
def export_tqsdk_opt_daily_data_to_csv(api, file):
    opt_basic_df = pd.read_csv('./doc/opt-basic/{}'.format(file), encoding='gbk')
    opt_basic_df = opt_basic_df[(opt_basic_df.delist_date >= 20180101)].copy()
    ...
    for i in range(0, len(code_list)):
        ...
        kd = DataDownloader(api, symbol_list=ts_code, dur_sec=86400,
                        start_dt=datetime(2018, 1, 1, 6, 0 ,0), end_dt=datetime(2024, 3, 18, 16, 0, 0), csv_file_name="./doc/daily-kline/opt/{}.daily.csv".format(ts_code))
        
        while not kd.is_finished():
            api.wait_update()
            print("{} progress: kline: %.2f%%".format(ts_code) % (kd.get_progress()))
        
        print("{} 日 k 线导出完毕，进度：{}%".format(ts_code, round((i + 1) / len(code_list) * 100, 2)))
...
def export_tqsdk_opt_tick_data_to_csv(api, file):
    opt_basic_df = pd.read_csv('./doc/opt-basic/{}'.format(file), encoding='gbk')
    opt_basic_df = opt_basic_df[(opt_basic_df.delist_date >= 20240222)].copy()
    opt_basic_df.sort_values(by='ts_code', ascending=True, inplace=True)
    opt_basic_df.reset_index(drop=True, inplace=True)
    code_list = opt_basic_df['ts_code'].tolist()
    ...
    for i in range(0, len(code_list)):
        ...
        kd = DataDownloader(api, symbol_list=ts_code, dur_sec=60,
                        start_dt=datetime(2024, 2, 22, 6, 0 ,0), end_dt=datetime(2024, 3, 22, 16, 0, 0), csv_file_name="./doc/tick-data/opt/{}.tick.csv".format(ts_code))
...
def main():
    ...
    # export_tqsdk_stock_daily_data_to_csv(api)
    
    # export_tqsdk_index_daily_data_to_csv(api)
    
    # path = './doc/opt-basic'
    # files = os.listdir(path)
    # for i in range(0, len(files)):
    #     file = files[i]
    #     export_tqsdk_opt_daily_data_to_csv(api, file)
    
    # path = './doc/opt-basic'
    # files = os.listdir(path)
    # for i in range(0, len(files)):
    #     file = files[i]
    #     export_tqsdk_opt_tick_data_to_csv(api, file)
