...
def get_trade_date_set():
    engine_ts = creat_mysql_engine('bond')
...
def get_all_stock_basic_data():
    engine_ts = creat_mysql_engine('stock')
    df = pro.stock_basic()
    write_data(engine_ts, 'stock_basic', 'stock', df)
    ...
    for i in range(0, len(ts_code)):
        ...
        for _ in range(60):
            try:
                ...
                if len(df):
                    write_data(engine_ts, 'cb_daily', 'bond', df)
                else:
...
def get_all_fut_basic_data():
    ...
    for exchange in futuresExchanges:
        ...
        write_data(engine_ts, 'fut_basic', 'futures', df)
    ...
    for i in range(0, len(ts_code)):
        ...
        for _ in range(60):
            try:
                ...
                if len(df):
                    write_data(engine_ts, 'fut_daily', 'futures', df)
                else:
    ...
    for i in range(0, len(date_df)):
        ...
        for _ in range(60):
            try:
                ...
                if len(warehouse_df):
                    write_data(engine_ts, 'fut_warehouse', 'futures', warehouse_df)
                else:
        ...
        for _ in range(60):
            try:
                ...
                if len(warehouse_df):
                    ...
                    write_data(engine_ts, 'fut_warehouse_sum', 'futures', warehouse_sum_df)
                else:
                    print('回调数据为空！查询日期:', dateStr)
            except:
                time.sleep(1)
