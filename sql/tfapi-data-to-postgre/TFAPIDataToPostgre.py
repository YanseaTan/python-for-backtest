...
def get_data(fut_code, index_type, symbol, start_date = '', end_date = ''):
    ...
    # print(data)
    # exit(1)
    engine_ts = creat_engine_with_database('future')
...
def update_all_data():
    ...
    for i in range(0, len(config_json)):
        ...
        for j in range(0, len(config_json[i]['index'])):
            ...
            sql = "select update_date from future.fut_funds where index_name = '{}' order by update_date desc limit 1;".format(index_name)
...
def merge_data(index_name_list, new_index_name):
    engine_ts = creat_engine_with_database('future')
    sql = "select fut_code, index_type from future.fut_funds where index_name = '{}' limit 1;".format(index_name_list[0])
    basic_df = read_data(engine_ts, sql)
    fut_code = basic_df.loc[0]['fut_code']
    index_type = basic_df.loc[0]['index_type']
    
    sql = "select update_date from future.fut_funds where index_name = '{}' order by update_date desc limit 1;".format(new_index_name)
    last_update_date_df = read_data(engine_ts, sql)
    value_dict = {}
    if len(last_update_date_df):
        last_update_date = last_update_date_df.loc[0]['update_date']
        for i in range(0, len(index_name_list)):
            ...
            sql = "select update_date, value from future.fut_funds where index_name = '{}' and update_date > '{}' order by update_date".format(index_name, last_update_date)
            value_df = read_data(engine_ts, sql)
            value_dict[i] = value_df
    else:
        for i in range(0, len(index_name_list)):
            index_name = index_name_list[i]
            sql = "select update_date, value from future.fut_funds where index_name = '{}' order by update_date".format(index_name)
            value_df = read_data(engine_ts, sql)
            value_dict[i] = value_df
...
def main():
    ...
    # data = fy_api.Get_Data('华东苯乙烯周度港口库存')
    # print(data)
